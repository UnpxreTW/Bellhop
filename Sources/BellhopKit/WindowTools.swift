...
/// `window_layout_list`）＋需要 macOS Accessibility 權限的動作面
/// （`window_focus`／`window_set_frame`／`window_restore_layout`，AX 整合見
/// ``WindowActions``）。`window_arrange` 的 typed schema 已定義於 ``plannedTools``
/// 但尚未曝光——它的主路徑是按原生「移動與調整大小」選單項，選單尋址
/// （AXIdentifier 是否存在且跨 App 穩定）尚待實機驗證，未驗前不曝光半套。
/// 視窗標題僅在宿主 app 已授 Screen Recording 權限時可讀（幾何與 owner
/// 資訊免權限）。
///
/// 本檔＝分派 ＋ 唯讀／快照面實作；需要 Accessibility 的動作面實作在
/// `WindowToolsActions.swift`、工具 schema 在 `WindowToolsSchemas.swift`。
enum WindowTools {
	...
	static func handle(name: String, arguments: [String: Value]?) async -> CallTool.Result {
		...
		case "window_restore_layout":
			await restoreLayout(arguments: arguments)
		case "window_arrange":
			errorResult("""
				\(name) is not available yet — it drives the native macOS "Move & \
				Resize" menu, and that menu addressing is not implemented. Use \
				window_set_frame for an exact frame in the meantime. Currently \
				available: window_list, window_save_layout, window_layout_list, \
	...
	static func displayFrames() -> [WindowFrame] {
	...
	/// 工具層失敗的統一回傳形（MCP 慣例：isError 而非丟例外）。
